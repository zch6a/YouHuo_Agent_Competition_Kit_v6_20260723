...
### 13.（已排除）`/elder3` 那两个 401 是量出来的，不是页面里的

留下这一条不是因为它是问题，是因为**下一个人很可能会再看到一次同样的输出**，
而它长得非常像一个真缺陷。

看到的是：

```
[今天] 控制台错误 0 · 失败请求 2 · load 2 次
     401 /api/v1/settings
     401 /api/v1/settings
```

`load 2 次` = 中途整页重载过，用户看得见闪一下。路径是 `common.js` 的身份
续期：两次 401 之后 `renew()` + `location.reload()`。

追了五个探针才定位到：**它只在端口 9001 上出现**。同一个脚本、同一份代码、
同一套 Chrome 参数，换成 9017 就是 `失败请求 0 · load 1 次`。成因是这个端口上
反复跑过服务——`Popen.terminate()` 只杀父进程，uvicorn 的工作进程会活下来
继续监听，新一轮起来时先应答的是**上一轮那个服务和它的旧库**，
里面自然没有这一轮新建的访客沙箱，于是 401。

排除过的（都不是）：service worker（第二次访问四页全 0 非 2xx）；
`/api/v1` 与 `/v2` 鉴权不同（`_actor` 和 `current_actor` 走同一个
`resolve_auth_token`）；访客令牌发错（抓过完整链路，`/v2/auth/visitor` 发的
`elder_token` 被后面五个 `/api/v1` 原样带上，全 200）。

教训和这一轮另一处是同一个：**探针脚本必须按端口收尸**，
`terminate()` 之后再 `netstat` + `taskkill /T` 一次。不然量到的是上一轮。
