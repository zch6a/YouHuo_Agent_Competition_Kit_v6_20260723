...
- `check_speech_text.mjs`：18项朗读文本规范化断言；
- `check_contrast.py`：6个页面×明暗两模式的 WCAG AA 与触控尺寸审计；
- `verify_features_v6.py` / `run_feature_audit.py`：122项逐功能端到端验收与 OpenAPI 覆盖率强制校验；
- `check_artifacts_v6.py`：文件、OpenAPI、Skill、HarmonyOS、报告与敏感产物检查；
- `verify_all.*`：确定性日常回归；
