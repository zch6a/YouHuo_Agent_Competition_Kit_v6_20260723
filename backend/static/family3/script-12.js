...
const reduceMotion=false;
  ...
  const intros = [
    {selector:'[data-family="today"]', delay:1750, text:'这里是「今天」。先看今天最重要的一件事，再看整体状态和需要家人确认的内容。'},
    {selector:'[data-family="todo"]', delay:1750, text:'这里是「待办」。提醒会按时间整理，也能新增、删除和确认事项。'},
    {selector:'#goCare', delay:1950, text:'这里是「照护中心」。生活节律、用药、身体、心情和安全，会放到一起看。'},
    {selector:'[data-family="mine"]', delay:1900, text:'这里是「我的」。做过的事、可信记录和个人入口都会留在这里。'},

    {selector:'#careTabs [data-care-panel="overview"]', delay:1900, text:'「概览」把五类照护信息收成一张图。先看整体，再沿着分支看细节。'},
    {selector:'#careTabs [data-care-panel="today"]', delay:1900, text:'「今天」按生活节律整理起床、午休、活动和晚间记录，方便一眼看懂今天。'},
    {selector:'#careTabs [data-care-panel="med"]', delay:1900, text:'「用药」会说明药名、剂量、服用时间，以及今天是否已经记录。'},
    {selector:'#careTabs [data-care-panel="body"]', delay:1900, text:'「身体」重点看血压、体温、体重、心率这些记录的变化，不只盯某一次数字。'},
    {selector:'#careTabs [data-care-panel="mood"]', delay:1900, text:'「心情」把每日问候、语音和家人记录整理成趋势，帮助了解最近的情绪状态。'},
    {selector:'#careTabs [data-care-panel="safety"]', delay:1900, text:'「安全」查看联系人、提醒、紧急联系和异常事件，让守护一直在但不打扰。'},
    {selector:'#careTabs [data-care-panel="trend"]', delay:1900, text:'「趋势」把最近十四天的变化连起来看，避免被单次波动带着走。'},

    {selector:'[data-vein-node="today"]', delay:0, text:'这个分支是「今天」。这里看起居和日常节律是不是和平常接近。'},
    {selector:'[data-vein-node="med"]', delay:0, text:'这个分支是「用药」。这里看长期用药和今天的服药记录。'},
    {selector:'[data-vein-node="body"]', delay:0, text:'这个分支是「身体」。体征、体检和就诊记录都从这里继续看。'},
    {selector:'[data-vein-node="mood"]', delay:0, text:'这个分支是「心情」。这里主要看最近两周的情绪和陪伴趋势。'},
    {selector:'[data-vein-node="safety"]', delay:0, text:'这个分支是「安全」。联系人、提醒和异常事件都集中在这里。'},

    {selector:'#familyFlow .flow-add', delay:0, text:'这个「＋」可以增加新的待办。填好时间和事项后，它会自动接到事项链里。'},
    {selector:'#familyFlow .flow-delete', delay:0, text:'这里可以删除一项待办。删除后，上下两项之间会自然重新连接。'},
    {selector:'#familyFlow .flow-item', delay:0, text:'这是一个待办气泡。时间、事项和完成状态都放在同一个节点里。'},
    {selector:'#careFlow .flow-item', delay:0, text:'这是生命节律记录。正常、需要关注、等待补充，会用不同状态表达。'},

    {selector:'.primary-action', delay:0, text:'这是当前最重要的操作入口。需要您真正确认的事情，会优先放在这里。'},
    {selector:'.secondary-action', delay:0, text:'这里可以快速查看今天的整体情况，再决定要不要继续深入。'},
    {selector:'.contact', delay:0, text:'这是「一键联系子女」。需要沟通时，可以直接从这里联系家人。'},
    {selector:'.app-switch [data-app="family"]', delay:1850, text:'现在切到「家人端」。这里主要处理确认、待办、照护查看和家庭记录。'},
    {selector:'.app-switch [data-app="care"]', delay:1950, text:'现在切到「照护中心」。这里更集中地查看老人近期状态和变化。'},

    {selector:'.rhythm-node', delay:0, text:'这是今天的一个时间节点。时间、发生的事情和当前状态会放在一起。'},
    {selector:'.medicine-seal', delay:0, text:'这是一条用药记录。可以看到药名、剂量、时间和今天的完成情况。'},
    {selector:'.body-metric', delay:0, text:'这是一个身体指标。这里更强调最近记录和趋势，而不是只看一个孤立数字。'},
    {selector:'.guard-node', delay:0, text:'这是一个安全守护项目。点开后可以继续查看具体设置和状态。'}
  ];
