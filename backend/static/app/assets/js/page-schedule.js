 ...
 * 只有一个数据源：
 *
 *   GET /api/v1/reminders               全表。顶上那张「接下来」也从它算，不走 /agenda
 *   GET /api/v1/reminders?kind=用药     筛某一类时多拉这一次，分类始终由后端说了算
 *
 * 为什么「接下来」不用 `GET /agenda`（它本来就是干这个的）：那个接口判「办完了」
 * 只看 COMPLETED / ACKNOWLEDGED，**取消掉的一条它照样当待进行回出来**，
 * 于是在这一页取消一件事之后，顶上会继续写着它。详见 `pickNext()` 上的说明。
 *
